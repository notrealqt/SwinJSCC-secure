...
class SwinJSCC_Encoder(nn.Module):
    ...
    def forward(self, x, snr, rate, model, preserve_gradients=False):
        B, C, H, W = x.size()
        device = x.get_device()
        ...
        if model == 'SwinJSCC_w_o_SAandRA':
            x = self.head_list(x)
            return x

        elif model == 'SwinJSCC_w_SA':
            snr_cuda = torch.tensor(snr, dtype=torch.float).to(device)
            snr_batch = snr_cuda.unsqueeze(0).expand(B, -1)
            for i in range(self.layer_num):
                if i == 0:
                    temp = self.sm_list[i](x if preserve_gradients else x.detach())
                else:
            ...
            for i in range(self.layer_num):
                if i == 0:
                    temp = self.sm_list1[i](x if preserve_gradients else x.detach())
                else:
